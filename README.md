...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our [CData MCP Server for SharePoint Excel Services](https://www.cdata.com/drivers/excelservices/download/mcp/). 
## Purpose
...
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/sharepoint-excel-services-mcp-server-by-cdata.git
      cd sharepoint-excel-services-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        ...
        * (Mac/Linux) `/Applications/CData JDBC Driver for SharePoint Excel Services/`
    * Run the command `java -jar cdata.jdbc.excelservices.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.excelservices.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `sharepoint-excel-services.prp`) using the following properties and format:
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "sharepoint_excel_services_get_tables",
        "arguments": {}
    }
}
```
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "sharepoint_excel_services_get_columns",
        "arguments": {
            "table":  "Account"
        }
    }
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "sharepoint_excel_services_run_query",
        "arguments": {
